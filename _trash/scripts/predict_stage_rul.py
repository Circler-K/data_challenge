...
def main():
    ...
    sub.to_excel(ROOT / "outputs" / f"{TEAM}_validation.xlsx",
                 index=False, sheet_name="Sheet1")
    res.to_csv(ROOT / "outputs" / "test_rul_stage.csv", index=False)
    print(f"\nsaved outputs/{TEAM}_validation.xlsx (Option B, stage-based)")
